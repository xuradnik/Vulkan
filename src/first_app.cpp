...
namespace lve
{
	...
	void FirstApp::createCommandBuffers() {
		...
		const int cmb = m_commandBuffers.size();
		for (int i = 0; i < cmb; i++) {
			...
			m_lve_pipeline->bind(m_commandBuffers[i]);
			this->m_lveModel->bind((m_commandBuffers[i]));
			this->m_lveModel->draw((m_commandBuffers[i]));
